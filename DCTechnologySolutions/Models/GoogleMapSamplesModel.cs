...
namespace DCTechnologySolutions.Models
{
    ...
    public class GoogleMapSamplesModel
    {
        Stadiums stadiums = new Stadiums();
        public GoogleMapSamplesModel()
        {
            //Locs.Add(new GoogleMapLocation { Name = "", Desc = "", Address = "", Longitude = 0, Latitude = 0 });
            Locs = new List<GoogleMapLocation>();
            ...
            stadiums.StadiumList = JsonConvert.DeserializeObject<List<Stadium>>(jsonStadiums);
            foreach (Stadium s in stadiums.StadiumList)
            {
                Locs.Add(new GoogleMapLocation { Name=s.team, Address = s.address, Latitude = s.lat, Longitude = s.lng, bInclude = false});
            }
        }

        public List<GoogleMapLocation> Locs { get; set; }
